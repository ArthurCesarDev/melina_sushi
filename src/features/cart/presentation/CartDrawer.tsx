...
const SAVED_ADDRESS_KEY = "melina-sushi-delivery-address";
...
type CartDrawerProps = {
  ...
  onFinish: (address: string, paymentMethod: string, notes: string, sauce: string) => void;
};
...
export default function CartDrawer({
  ...
  useEffect(() => {
    try {
      const savedAddress = window.localStorage.getItem(SAVED_ADDRESS_KEY);
      if (savedAddress) setAddress(savedAddress);
    } catch {
      // O carrinho continua funcionando caso o navegador bloqueie o localStorage.
    }
  }, []);

  const handleRememberAddressChange = (remember: boolean) => {
    setRememberAddress(remember);

    if (!remember) {
      try {
        window.localStorage.removeItem(SAVED_ADDRESS_KEY);
      } catch {
        // O estado visual ainda é atualizado se o localStorage estiver indisponível.
      }
    }
  };
  ...
  const handleConfirm = () => {
    ...
    try {
      if (rememberAddress) {
        window.localStorage.setItem(SAVED_ADDRESS_KEY, address.trim());
      } else {
        window.localStorage.removeItem(SAVED_ADDRESS_KEY);
      }
    } catch {
      // Falhas ao salvar não devem impedir o cliente de finalizar o pedido.
    }
                ...
                <div>
                  ...
                  <label className="mt-2 flex cursor-pointer items-start gap-2 text-xs text-gray-600">
                    <input
                      ...
                      onChange={(e) => handleRememberAddressChange(e.target.checked)}
                      className="mt-0.5 h-4 w-4 accent-[#FF5722]"
                    />
                    <span>Lembrar meu endereço neste dispositivo</span>
                  </label>
                </div>
